...
type SortDecision = "keep" | "donate" | "sell" | "toss";
...
export const tossAI = onRequest({ secrets: ["OPENAI_API_KEY"], cors: true }, async (req, res) => {
  ...
  try {
    const body = req.body as AnalyzeRequest | ChatRequest;
...
async function analyzeItem(input: AnalyzeRequest) {
        ...
        content: [
          {
            type: "input_text",
            text:
              "You are Toss, an enthusiastic decluttering assistant. Analyze one item photo. Reply shortly. Return only valid JSON matching the requested fields. Recommend exactly one of keep, donate, sell, toss. Give a realistic USD resale range.",
          },
        ],
      },
      {
...
async function chatWithToss(input: ChatRequest) {
  const response = await openai.responses.create({
    model,
    input: [
      {
        ...
        content: [
          {
            type: "input_text",
            text:
              "You are Toss, a short, enthusiastic decluttering assistant. Help the user decide what to keep, donate, sell, or toss. Return only JSON with reply and optional memoryUpdate.",
          },
        ],
      },
      {
...
async function writeMemory(uid: string, text: string) {
